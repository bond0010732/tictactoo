...
const startQuizGame66 = (httpServer) => {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('joinGame', ({ playerName, roomName, userId, totalBet }) => {
          ...
          console.log(`${playerName} joined roomss ${roomName} as Player ${playerNumber}`);
    ...
    console.error('Error updating winner\'s balance or saving to DB:', error.message);
        ...
        socket.on('placeBet', async ({ roomName, userId, playerNumber, playerName, betAmount }) => {
            ...
            if (playerNumber === 1) {
              ...
              try {
                const newBet = new BetModelQuiz({
                  roomId: roomName,
                  playerName,
                  betAmount,
                });
                await newBet.save();
                console.log('Bet saved to BetModel database:', newBet);
              } catch (error) {
        ...
        //         rooms[roomName].player1Bet = betAmount;
        //         rooms[roomName].player1UserId = userId;
        //         playerName = playerName || userId;  // Use playerName or fall back to userId
        //     } else if (playerNumber === 2) {
        //         playerBet = betAmount;
        //         otherPlayerBet = rooms[roomName].player1Bet;
        //         playerUserId = userId;
        ...
        //     try {
        //         // Save Player 1's bet to BetModel (if player number 1)
        //         if (playerNumber === 1) {
        //             const newBet = new BetModelQuiz({
        //                 roomId: roomName,
        //                 playerName,
        ...
        //             });
        //             await newBet.save();
        //             console.log('Bet saved to BetModel:', newBet);
        //         }
        
        //         // Save both Player 1 and Player 2's bets to BetCashModel
        //         const newBetCash = new BetCashModel({
        //             roomId: roomName,
        //             player1Bet: rooms[roomName].player1Bet,
        //             player2Bet: rooms[roomName].player2Bet,
        //             player1UserId: rooms[roomName].player1UserId,
        //             player2UserId: rooms[roomName].player2UserId,
        //             playerName,
        //             betAmount,
        //         });
        //         await newBetCash.save();
        //         console.log('Bets saved to BetCashModel:', newBetCash);
        //     } catch (error) {
...
const generateRandomRoomName = () => {
    const length = 6; // Length of the random room name
...
module.exports = startQuizGame66;
